extern "C" __global__ void compute_firth_components(const double* phenotype,
  ...
  for (unsigned long long sample_index = thread_index; sample_index < sample_count; sample_index += kernel_block_size) {
    ...
    const double linear_predictor = offset[element_index] + genotype_value * lane_beta;
    double probability;
    ...
    const double information_diagonal = genotype_value * genotype_value * weight;
    information_sum += information_diagonal;
    adjustment_numerator_sum += genotype_value * information_diagonal * (0.5 - probability);
    score_sum += genotype_value * (phenotype_value - probability);
    deviance_sum += phenotype_value > 0.5 ? -2.0 * log(probability) : -2.0 * log1p(-probability);
  }

  for (unsigned int offset_width = warp_size / 2; offset_width != 0; offset_width /= 2) {
    information_sum += __shfl_down_sync(full_warp_mask, information_sum, offset_width);
    adjustment_numerator_sum += __shfl_down_sync(full_warp_mask, adjustment_numerator_sum, offset_width);
    score_sum += __shfl_down_sync(full_warp_mask, score_sum, offset_width);
    deviance_sum += __shfl_down_sync(full_warp_mask, deviance_sum, offset_width);
  }
  ...
  information_sum = warp_lane_index < warp_count ? warp_information[warp_lane_index] : 0.0;
  adjustment_numerator_sum = warp_lane_index < warp_count ? warp_adjustment_numerator[warp_lane_index] : 0.0;
  score_sum = warp_lane_index < warp_count ? warp_score[warp_lane_index] : 0.0;
  deviance_sum = warp_lane_index < warp_count ? warp_deviance[warp_lane_index] : 0.0;
  for (unsigned int offset_width = warp_size / 2; offset_width != 0; offset_width /= 2) {
    information_sum += __shfl_down_sync(full_warp_mask, information_sum, offset_width);
    adjustment_numerator_sum += __shfl_down_sync(full_warp_mask, adjustment_numerator_sum, offset_width);
    score_sum += __shfl_down_sync(full_warp_mask, score_sum, offset_width);
  ...
  const double lane_score_adjustment = adjustment_numerator_sum / lane_information;
  const double lane_penalized_deviance = non_active_deviance[lane_index] + deviance_sum - log(lane_information);
  ...
  valid[lane_index] = isfinite(lane_information) && lane_information > minimum_variance[lane_index] &&
