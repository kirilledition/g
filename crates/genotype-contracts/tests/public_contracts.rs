...
fn build_metadata_fixture(range: std::ops::Range<usize>) -> MetadataFixture {
    ...
    MetadataFixture { columns: VariantMetadataColumns::new(store, range), chromosome_x }
}
...
fn metadata_store_debug_checks_parallel_column_lengths() {
    let _ = VariantMetadataStore::from_parts(
        vec![Arc::from("22"), Arc::from("A")].into_boxed_slice(),
...
#[cfg(debug_assertions)]
#[test]
#[should_panic(expected = "variant identifier offsets must be monotonic")]
fn metadata_store_debug_checks_identifier_offset_order() {
    let _ = VariantMetadataStore::from_parts(
        vec![Arc::from("22"), Arc::from("A")].into_boxed_slice(),
        vec![0, 0].into_boxed_slice(),
        "rs1".to_string().into_boxed_str(),
        vec![0, 4, 3].into_boxed_slice(),
        vec![101, 102].into_boxed_slice(),
...
#[cfg(debug_assertions)]
#[test]
#[should_panic(expected = "metadata dictionary codes must address the shared dictionary")]
fn metadata_store_debug_checks_dictionary_codes() {
    let _ = VariantMetadataStore::from_parts(
        vec![Arc::from("22"), Arc::from("A")].into_boxed_slice(),
...
#[cfg(debug_assertions)]
#[test]
#[should_panic(expected = "metadata range must stay within the shared store")]
fn metadata_columns_debug_check_the_selected_range() {
    let store = Arc::new(VariantMetadataStore::from_parts(
        vec![Arc::from("22"), Arc::from("A")].into_boxed_slice(),
        vec![0].into_boxed_slice(),
        "rs1".to_string().into_boxed_str(),
    ...
    let _ = VariantMetadataColumns::new(store, 0..2);
}
