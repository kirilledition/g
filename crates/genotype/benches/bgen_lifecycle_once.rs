...
use g_genotype::{
    BGEN_OWNED_SNAPSHOT_MAXIMUM_BYTE_COUNT, BgenContentSelector, BgenReaderCore, ChunkStatisticsPolicy,
...
fn open_benchmark_reader(bgen_path: &Path, content_selector: BgenContentSelector) -> BgenReaderCore {
    BgenReaderCore::open_request(BgenOpenRequest { locator: bgen_path, content_selector: Some(content_selector) })
        .expect("content-selected native Rust BGEN lifecycle reader should open")
}
...
fn main() {
    ...
    if !cfg!(feature = "benchmark-positioned-source") {
        assert!(
            source_byte_count <= BGEN_OWNED_SNAPSHOT_MAXIMUM_BYTE_COUNT,
            "content-selected lifecycle benchmark requires an owned-snapshot input",
        );
    }
    ...
    let priming_reader = primed_reopen.then(|| open_benchmark_reader(&bgen_path, content_selector));
    drop(priming_reader);
    ...
    println!("content_sha256={}", content_selector.content_sha256);
    ...
    println!(
        "snapshot_resolution={}",
        match reader.source_provenance().resolution {
            BgenSnapshotResolution::CapturedFromLocator => "captured_from_locator",
            BgenSnapshotResolution::ProcessSnapshotCache => "process_snapshot_cache",
        }
    );
