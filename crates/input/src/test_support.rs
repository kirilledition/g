...
impl TemporaryDirectory {
    pub(crate) fn new(test_name: &str) -> Self {
        ...
        let path =
            std::env::temp_dir().join(format!("g-input-{test_name}-{}-{directory_identifier}", std::process::id()));
        std::fs::create_dir(&path).expect("temporary input-test directory should be created");
        Self { path }
    }

    pub(crate) fn write(&self, file_name: &str, contents: &str) -> PathBuf {
        let path = self.path.join(file_name);
        std::fs::write(&path, contents).expect("temporary input-test file should be written");
        path
    }
