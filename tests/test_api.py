...
def test_linear_uses_public_api_defaults() -> None:
    ...
    with (
        patch("g.api.jax_setup.configure_jax_device") as mock_configure_jax_device,
        patch("g.api.engine.iter_linear_output_frames", return_value=iter(())) as mock_iter_linear_output_frames,
        patch("g.api.engine.write_frame_iterator_to_tsv") as mock_write_frame_iterator_to_tsv,
    ):
        artifacts = linear(
            bfile="dataset",
            pheno="phenotype.tsv",
...
def test_logistic_uses_mode_specific_defaults() -> None:
    ...
    with (
        patch("g.api.jax_setup.configure_jax_device") as mock_configure_jax_device,
        patch("g.api.engine.iter_logistic_output_frames", return_value=iter(())) as mock_iter_logistic_output_frames,
        patch("g.api.engine.write_frame_iterator_to_tsv") as mock_write_frame_iterator_to_tsv,
    ):
        artifacts = logistic(
            bfile="dataset",
...
def test_linear_supports_bgen_input() -> None:
    ...
    with (
        patch("g.api.jax_setup.configure_jax_device"),
        patch("g.api.engine.iter_linear_output_frames", return_value=iter(())) as mock_iter_linear_output_frames,
        patch("g.api.engine.write_frame_iterator_to_tsv"),
    ):
        linear(
            bfile=None,
            bgen="dataset.bgen",
...
def test_regenie2_linear_uses_bgen_input_and_prediction_list() -> None:
    ...
    with (
        patch("g.api.jax_setup.configure_jax_device") as mock_configure_jax_device,
        patch("g.api.engine.iter_regenie2_linear_output_frames", return_value=iter(())) as mock_iterator,
        patch("g.api.engine.write_frame_iterator_to_tsv") as mock_write_frame_iterator_to_tsv,
    ):
