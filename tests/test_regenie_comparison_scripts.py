...
def test_deep_profile_arguments_can_focus_trait_and_device_grid() -> None:
    arguments = deep_profile.build_arguments_from_overrides(
        [
            "tool.trait_types=[binary]",
            "tool.devices=[gpu]",
        ]
    )

    assert arguments.trait_types == "binary"
    assert arguments.devices == "gpu"
    assert deep_profile.parse_profile_trait_types(arguments.trait_types) == ("binary",)
    assert deep_profile.parse_profile_devices(arguments.devices) == ("gpu",)
