...
- `crates/engine/src/` now owns the first native coordinator scaffold:
  ...
  benchmark, and the BGEN-backed `Regenie2RunEngineCore`/chunk planning core
  that used to live under `src/pipeline/`. Production queues, output writer
  lifecycle, cleanup, telemetry emission, and the PyO3/JAX association backend
