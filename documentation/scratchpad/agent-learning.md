...
- `crates/engine/src/` now owns the first native coordinator scaffold:
  ...
  policy, callback queue-limit policy, callback queue stage and operation
  observation policy, variant-major dosage batch handoff planning, result
  ...
  shutdown timeout, stop/join-planning, and finish/abort shutdown policy,
  callback worker stop poll-timeout policy, callback worker stop-attempt
  decision policy, and BGEN delivery method selection also live in `g-engine`;
  the same crate also owns the callback worker backpressure poll-timeout
  policy, binary correction summary counter accumulation, and callback
  progress/chunk identity state. Native multi-trait committed-chunk write
  selection is also owned by `g-engine`.
