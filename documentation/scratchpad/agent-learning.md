...
- `crates/runtime/src/` now owns pure runtime policy/state helpers: logging
  ...
  cap/counter/envelope/progress-throttle state, telemetry JSON-line emission,
  run lifecycle event payload/rendering policy, shutdown signal metadata and
  controller state, stage timing state, exact-stage timing policy,
