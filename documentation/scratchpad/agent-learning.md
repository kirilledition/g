...
- `crates/runtime/src/` now owns pure runtime policy/state helpers: logging
  ...
  cap/counter/envelope/progress-throttle state, telemetry
  event/close-event/JSON-line emission, run lifecycle event payload/rendering
  policy, shutdown signal metadata and controller state, stage timing state,
  exact-stage timing policy, stage-timing/profile JSON payloads and file writes,
  run metadata payloads, and the logging/Rayon/JAX process runtime state handle.
  Root PyO3 adapters still own other side effects and Python-only JAX setup
