...
| Execution planning | `src/g/execution_plan.py`, `src/g/engine/backend_planner.py` | `crates/plan/src/`, `crates/interface/src/plan_request.rs`, native metadata helpers | `g-plan`, then `g-engine` | Rust | Started: `g-interface` compiles resolved config into `g-plan::RunRequest`; Python adapts the request into legacy dataclasses while `PreparedRunPlan` and manifest authority remain pending | `tests/test_backend_planner.py`, `tests/test_regenie2_pipeline.py`, `cargo test -p g-plan`, `cargo test -p g-interface` |
