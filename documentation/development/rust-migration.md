...
nonconverged counts before calling `g-engine` policy helpers. Python callback
diagnostics still materialize the JAX convergence value, but no longer use
`np.ravel` or `np.count_nonzero` for that production policy path; the Python
...
fake and transitional test writer sessions.
