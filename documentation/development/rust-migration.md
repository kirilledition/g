...
| Execution planning | `src/g/execution_plan.py`, `src/g/engine/backend_planner.py` | `crates/plan/src/host_policy.rs`, `crates/interface/src/run_validation.rs`, native metadata helpers | `g-plan`, then `g-engine` | Rust | Started: deterministic host policy payloads live in `g-plan`; Python still assembles the full plan | `tests/test_backend_planner.py`, `tests/test_regenie2_pipeline.py`, `cargo test -p g-plan` |
