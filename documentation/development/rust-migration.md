...
exported from `_core`.
Preflight finite-array checks, SVD-backed covariate-rank validation, and binary
phenotype coding/case-control scans now execute in the root PyO3 adapter over
NumPy buffers before calling `g-engine` policy helpers. Python preflight no
longer uses `np.isfinite`, `np.linalg.matrix_rank`, `np.unique`, or
`np.count_nonzero` for those production checks, and the Python architecture
