...
| --- | --- | --- | --- | --- | --- | --- |
| CLI/TOML/config frontend | `src/g/cli.py`, `src/g/interface/config.py` | `crates/interface/src/`, `crates/cli/src/` | `g-cli`, `g-interface` | Rust | Native frontend prototype added: Cargo binary `g` owns help, parse errors, and run-config validation through `g-interface`; validated run execution is intentionally refused until the Python/JAX backend embedding boundary is designed, while the Python console script remains production full-run entrypoint | `cargo test -p g-cli`, `tests/test_interface.py`, CLI help/package smoke |
