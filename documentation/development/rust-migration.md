...
| Runtime, telemetry, timing, shutdown | `src/g/runner/`, `src/g/engine/telemetry.py`, `src/g/engine/timing.py`, `src/g/engine/shutdown.py` | `crates/runtime/src/`, root PyO3 adapters | `g-runtime` | Rust | Started: pure runtime policy, telemetry path policy, telemetry session cap/counter/envelope state, run lifecycle event payload/rendering policy, shutdown metadata/controller state, timing state, exact-stage timing policy, stage-timing JSON payloads, run metadata, profile summary payloads, and logging/Rayon/JAX process state handle extracted; Python still owns side effects and JAX setup lifecycle | `tests/test_telemetry.py`, `tests/test_timing.py`, shutdown tests, `cargo test -p g-runtime` |
