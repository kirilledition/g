...
```bash
data/regenie2_binary_chr22_gpu.g/attempts/attempt-<id>/trait_0001_phenotype_binary/
```
...
- JAX persistent compilation cache defaults to
  `<platform temporary directory>/<user>/g-jax-cache`; use TOML
  `[compute].jax_cache_dir` for an explicit cache path.
