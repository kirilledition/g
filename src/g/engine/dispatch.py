"""Dispatch REGENIE step 2 scans over genotype chunks."""
...
BinaryChunkComputeFunction = typing.Callable[
    [models.Regenie2BinaryChromosomeState, jax.Array, types.RegenieBinaryCorrection], models.Regenie2BinaryChunkResult
]
compute_regenie2_binary_chunk = typing.cast(
    "BinaryChunkComputeFunction", regenie2_binary.compute_regenie2_binary_chunk_from_chromosome_state
)
...
def iter_regenie2_linear_output_frames(
    *,
...
) -> collections.abc.Iterator[engine_types.Regenie2ChunkAccumulator]:
    """Yield linear REGENIE step 2 result accumulators for one BGEN source."""
    ...
    with genotype_reader:
        ...
        regenie2_linear_state = regenie2_linear.prepare_regenie2_linear_state(
            covariate_matrix=aligned_sample_data.covariate_matrix, phenotype_vector=aligned_sample_data.phenotype_vector
        )
        ...
        for source_chunk in chunk_iterator:
            for current_chunk in chromosome_chunks.split_dosage_genotype_chunk_with_reader_metadata(
                source_chunk, genotype_reader
            ):
                ...
                if chromosome != current_chromosome:
                    ...
                    current_chromosome_state = regenie2_linear.prepare_regenie2_linear_chromosome_state(
                        regenie2_linear_state, current_loco_predictions
                    )
                    current_chromosome = chromosome
                assert current_chromosome_state is not None
                with profiling.profiled_regenie2_linear_chunk_step(chunk_number):
                    regenie2_linear_result = regenie2_linear.compute_regenie2_linear_chunk_from_chromosome_state(
                        chromosome_state=current_chromosome_state, genotype_matrix=current_chunk.genotypes
                    )
                    yield engine_types.Regenie2ChunkAccumulator(
...
def iter_regenie2_binary_output_frames(
    *,
    ...
    """Yield binary REGENIE step 2 result accumulators for one BGEN source."""
    if genotype_source_config.source_format != types.GenotypeSourceFormat.BGEN:
    ...
    with genotype_reader:
        ...
        regenie2_binary_state = regenie2_binary.prepare_regenie2_binary_state(
            covariate_matrix=aligned_sample_data.covariate_matrix, phenotype_vector=aligned_sample_data.phenotype_vector
        )
        prediction_source = regenie.load_prediction_source(prediction_list_path, phenotype_name)
        chunk_iterator = source.iter_dosage_genotype_chunks_from_source(
        ...
        for source_chunk in chunk_iterator:
            for current_chunk in chromosome_chunks.split_dosage_genotype_chunk_with_reader_metadata(
                ...
                if chromosome != current_chromosome:
                    ...
                    current_chromosome_state = regenie2_binary.prepare_regenie2_binary_chromosome_state(
                        regenie2_binary_state, loco_offset
                    )
                    current_chromosome = chromosome
                assert current_chromosome_state is not None
                with profiling.profiled_regenie2_binary_chunk_step(chunk_number):
                    result = compute_regenie2_binary_chunk(
                        current_chromosome_state, current_chunk.genotypes, correction
                    )
                    yield engine_types.Regenie2ChunkAccumulator(
                        metadata=current_chunk.metadata,
                        allele_one_frequency=current_chunk.allele_one_frequency,
