...
from g import _core
from g.engine.native_dispatch import events, timing
...
def finish_callback_drain(
    *,
    ...
    events.record_callback_drain_started()
    callback.finish()
...
def finish_writer_session(
    *,
    ...
    events.record_writer_session_finish_started()
...
def finish_writer_sessions(
    *,
    ...
    writer_finish_start_time = time.perf_counter()
    events.record_writer_sessions_finish_started(
...
def finish_writer_session_interrupted(
    *,
    ...
    writer_finish_start_time = time.perf_counter()
    events.record_writer_session_interrupted_flush_started(shutdown_request)
...
def finish_writer_sessions_interrupted(
    *,
    ...
    writer_finish_start_time = time.perf_counter()
    events.record_writer_sessions_interrupted_flush_started(
        shutdown_request,
        requested_thread_count=writer_finish_thread_count,
        writer_session_count=len(writer_sessions),
    )
