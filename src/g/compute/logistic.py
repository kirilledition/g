...
def initialize_full_model_coefficients(
    ...
    sample_covariate_pair_matrix = jnp.einsum("np,nq->npq", covariate_matrix, covariate_matrix)
    covariate_pair_matrix = sample_covariate_pair_matrix.reshape((covariate_matrix.shape[0], -1))
    flattened_information_matrix = observation_mask_float @ covariate_pair_matrix
    covariate_information_matrix = flattened_information_matrix.reshape(
        (-1, covariate_matrix.shape[1], covariate_matrix.shape[1])
    )
