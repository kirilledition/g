...
@functools.partial(
    jax.jit,
    static_argnames=("correction_plan", "kernel_config", "candidate_capacity", "score_dtype"),
    donate_argnames=("packed_probability_pairs_by_variant",),
)
def apply_device_candidate_corrections_firth_packed8_variant_major_with_capacity(
    chromosome_state: regenie2_binary_state.Regenie2BinaryChromosomeState,
    ...
    """Apply Firth corrections after gathering and decoding only packed8 candidate rows."""
    candidate_mask = result.extra_code == types.BinaryExtraCode.FIRTH.value
    fallback_count = jnp.sum(candidate_mask, dtype=jnp.int32)
    firth_batch_size = kernel_config.firth_candidate.batch_size
    ...
    candidate_packed_probability_pairs_by_variant = jnp.take(
        packed_probability_pairs_by_variant,
        flat_fallback_indices,
        axis=0,
    )
    raw_candidate_genotype_matrix_by_variant = (
        compute_genotype.decode_packed8_probability_pairs_to_variant_major_dosage(
            candidate_packed_probability_pairs_by_variant,
            score_dtype,
        )
    )
    prepared_batch = regenie2_binary_firth_batch.prepare_firth_candidate_batch_from_raw_candidate_genotypes(
        chromosome_state=chromosome_state,
        batch_plan=batch_plan,
        raw_candidate_genotype_matrix_by_variant=raw_candidate_genotype_matrix_by_variant,
        score_beta=result.beta,
        sparse_candidate_mask=sparse_candidate_mask,
        kernel_config=kernel_config,
    )
...
def apply_device_candidate_corrections_firth_packed8_variant_major(
    chromosome_state: regenie2_binary_state.Regenie2BinaryChromosomeState,
    packed_probability_pairs_by_variant: jax.Array,
    result: regenie2_binary_result.Regenie2BinaryScoreChunkResult,
    correction_plan: types.BinaryCorrectionPlan,
    ...
    """Select packed8 Firth capacity before gathering and decoding candidate rows."""
    candidate_mask = result.extra_code == types.BinaryExtraCode.FIRTH.value
    candidate_count_start_time = time.perf_counter() if stage_duration_recorder is not None else 0.0
    fallback_count = regenie2_binary_candidate_planning.count_firth_candidates_on_host(candidate_mask)
    if stage_duration_recorder is not None:
    ...
    return apply_device_candidate_corrections_firth_packed8_variant_major_with_capacity(
        chromosome_state=chromosome_state,
...
def apply_device_candidate_corrections_packed8_variant_major(
    chromosome_state: regenie2_binary_state.Regenie2BinaryChromosomeState,
    packed_probability_pairs_by_variant: jax.Array,
    result: regenie2_binary_result.Regenie2BinaryScoreChunkResult,
    correction_plan: types.BinaryCorrectionPlan,
    ...
    """Apply binary candidate corrections for packed8 variant-major probability pairs."""
    if correction_plan.method == types.BinaryFallbackMethod.SCORE_ONLY:
        return result
    if correction_plan.method == types.BinaryFallbackMethod.FIRTH:
        message = "Exact REGENIE --firth without --approx is not implemented yet. Use --firth --approx."
    ...
    if correction_plan.method == types.BinaryFallbackMethod.SPA:
        message = "SPA fallback is not implemented yet. Omit --spa for score-test-only output."
        raise NotImplementedError(message)
    return apply_device_candidate_corrections_firth_packed8_variant_major(
        chromosome_state=chromosome_state,
        packed_probability_pairs_by_variant=packed_probability_pairs_by_variant,
        result=result,
        correction_plan=correction_plan,
