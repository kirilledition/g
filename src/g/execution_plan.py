...
class PhenotypeRunPlan:
    ...
    Attributes:
        ...
        final_regenie_path: User-facing REGENIE text path, when requested.
...
def build_phenotype_run_plan(
    *,
    ...
    if output_plan.output_format in {types.OutputFormat.REGENIE, types.OutputFormat.BOTH}:
        final_regenie_path = output_plan.output_prefix.with_name(
            f"{output_plan.output_prefix.name}_{phenotype_name}.regenie"
        )
    return PhenotypeRunPlan(
        ...
        final_regenie_path=final_regenie_path,
    )
