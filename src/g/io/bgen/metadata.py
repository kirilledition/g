"""BGEN variant metadata normalization helpers."""
...
class OpenBgenHandle(typing.Protocol):
    """Legacy open-bgen metadata attributes used by compatibility helpers."""
...
class CoreBgenHandle(typing.Protocol):
    """Native BGEN core metadata methods used by the reader wrapper."""

    def variant_metadata_slice(self, variant_start: int, variant_stop: int) -> typing.Any:
        """Return native metadata arrays for one variant slice."""


@dataclass(frozen=True)
class CoreVariantMetadata:
    """Normalized metadata lists returned by the native BGEN core.

    Attributes:
        chromosome_values: Chromosome identifiers.
        variant_identifier_values: Variant identifiers.
        position_values: Genomic positions.
        allele_one_values: Counted allele values.
        allele_two_values: Reference allele values.
...
def resolve_variant_identifier_values(
    variant_identifier_values: npt.NDArray[np.str_], rsid_values: npt.NDArray[np.str_]
) -> npt.NDArray[np.str_]:
...
def build_bgen_variant_table_arrays(bgen_handle: OpenBgenHandle) -> reader.VariantTableArrays:
    """Build normalized variant metadata arrays from a legacy open-bgen handle."""
...
def build_bgen_variant_table(bgen_handle: OpenBgenHandle) -> pl.DataFrame:
    """Build a normalized variant metadata table from a legacy open-bgen handle."""
...
def build_variant_table_arrays_from_core_metadata(variant_metadata: CoreVariantMetadata) -> reader.VariantTableArrays:
    """Build normalized metadata arrays from native core metadata lists."""
    return reader.VariantTableArrays(
...
def build_variant_table_from_arrays(variant_table_arrays: reader.VariantTableArrays) -> pl.DataFrame:
    """Build a normalized variant metadata table from metadata arrays."""
    return pl.DataFrame(
        {
...
def build_variant_table_from_core_metadata(variant_metadata: CoreVariantMetadata) -> pl.DataFrame:
    """Build a normalized variant metadata table from native core metadata."""
...
def build_core_variant_metadata(
    core_reader: CoreBgenHandle, variant_start: int, variant_stop: int
) -> CoreVariantMetadata:
    """Read one normalized metadata slice from the native core reader."""
    ...
    return CoreVariantMetadata(
        list(chromosome_values),
        list(variant_identifier_values),
        list(position_values),
        list(allele_one_values),
        list(allele_two_values),
    )
