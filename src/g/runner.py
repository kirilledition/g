...
class RunArtifacts:
    ...
    Attributes:
        ...
        final_regenie: Finalized REGENIE text output path.
...
def finalize_phenotype_run(
    *,
    ...
    """Finalize metadata and optional REGENIE text output for one phenotype."""
    config.write_toml(regenie_config, phenotype_run_plan.effective_config_path)
    extend_run_manifest(
        plan=plan,
        phenotype_run_plan=phenotype_run_plan,
    )
    if phenotype_run_plan.final_regenie_path is not None:
        output.finalize_chunks_to_regenie_text(
            phenotype_run_plan.output_run_paths,
            phenotype_run_plan.final_regenie_path,
        )
    return RunArtifacts(
        output_run_directory=phenotype_run_plan.output_run_paths.run_directory,
        final_parquet=final_parquet_path,
        final_regenie=phenotype_run_plan.final_regenie_path,
