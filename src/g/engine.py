...
def concatenate_linear_results(
    ...
    # Fetch list of JAX arrays directly to host, then concatenate on CPU using numpy
    # This avoids unnecessary JAX tracing/JIT overhead and reduces peak device memory usage.
    host_values = jax.device_get(
        {
    ...
    # Concatenate on host
    for key, val in host_values.items():
        host_values[key] = np.concatenate(val)
