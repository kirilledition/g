...
def build_regenie2_linear_chunk_payload_batch(
    ...
    host_values = {key: np.concatenate(value_list) for key, value_list in host_values_lists.items()}
...
def concatenate_linear_results(
    ...
    # Extract JAX arrays into lists, transfer them to host in ONE device_get
    # to avoid JAX tracing/JIT overhead of jnp.concatenate
    host_values_lists = jax.device_get(
        {
