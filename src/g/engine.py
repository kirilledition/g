...
def build_regenie2_linear_chunk_payload_batch(
    ...
    # Memory and trace optimization: Transfer lists of JAX arrays directly to the host
    # and use numpy to concatenate them. This avoids massive JAX tracing/JIT overhead
    # compared to calling jnp.concatenate on a dynamic list of device arrays.
    host_values = jax.device_get(
        {
    ...
    host_allele_one_frequency = np.concatenate(host_values["allele_one_frequency"])
    host_observation_count = np.concatenate(host_values["observation_count"])
    host_beta = np.concatenate(host_values["beta"])
    host_standard_error = np.concatenate(host_values["standard_error"])
    host_chi_squared = np.concatenate(host_values["chi_squared"])
    host_log10_p_value = np.concatenate(host_values["log10_p_value"])
    host_valid_mask = np.concatenate(host_values["valid_mask"])
        ...
        payloads.append(
            Regenie2LinearChunkPayload(
                ...
                allele_one_frequency=np.ascontiguousarray(
                    host_allele_one_frequency[variant_offset:next_variant_offset]
                ),
                observation_count=np.ascontiguousarray(host_observation_count[variant_offset:next_variant_offset]),
                beta=np.ascontiguousarray(host_beta[variant_offset:next_variant_offset]),
                standard_error=np.ascontiguousarray(host_standard_error[variant_offset:next_variant_offset]),
                chi_squared=np.ascontiguousarray(host_chi_squared[variant_offset:next_variant_offset]),
                log10_p_value=np.ascontiguousarray(host_log10_p_value[variant_offset:next_variant_offset]),
                is_valid=np.ascontiguousarray(host_valid_mask[variant_offset:next_variant_offset]),
            )
        )
    ...
    return pl.DataFrame(
        {
            ...
            "allele_one_frequency": np.concatenate(host_values["allele_one_frequency"]),
            "observation_count": np.concatenate(host_values["observation_count"]),
            "beta": np.concatenate(host_values["beta"]),
            "standard_error": np.concatenate(host_values["standard_error"]),
            "t_statistic": np.concatenate(host_values["test_statistic"]),
            "p_value": np.concatenate(host_values["p_value"]),
            "is_valid": np.concatenate(host_values["valid_mask"]),
        }
    )
    ...
    return pl.DataFrame(
        {
            ...
            "beta": np.concatenate(host_values["beta"]),
            "standard_error": np.concatenate(host_values["standard_error"]),
            "z_statistic": np.concatenate(host_values["test_statistic"]),
            "p_value": np.concatenate(host_values["p_value"]),
            "firth_flag": format_logistic_method_codes(np.concatenate(host_values["method_code"])),
            "error_code": format_logistic_error_codes(np.concatenate(host_values["error_code"])),
            "converged": np.concatenate(host_values["converged_mask"]),
            "iteration_count": np.concatenate(host_values["iteration_count"]),
            "is_valid": np.concatenate(host_values["valid_mask"]),
        }
    )
    ...
    return pl.DataFrame(
        {
            ...
            "beta": np.concatenate(host_values["beta"]),
            "standard_error": np.concatenate(host_values["standard_error"]),
            "chi_squared": np.concatenate(host_values["chi_squared"]),
            "log10_p_value": np.concatenate(host_values["log10_p_value"]),
            "is_valid": np.concatenate(host_values["valid_mask"]),
        }
    )
