...
def build_regenie2_linear_chunk_payload_batch(
    ...
    # Performance optimization: Passing a dict of lists directly to jax.device_get
    # avoids the significant tracing and JIT overhead of jnp.concatenate for variable
    # sized array lists, and reduces peak device memory usage.
    host_values = jax.device_get(
        {
    ...
    for i, chunk_accumulator in enumerate(chunk_accumulators):
        ...
        payloads.append(
            Regenie2LinearChunkPayload(
                ...
                allele_one=metadata.allele_one,
                allele_two=metadata.allele_two,
                allele_one_frequency=np.ascontiguousarray(host_values["allele_one_frequency"][i]),
                observation_count=np.ascontiguousarray(host_values["observation_count"][i]),
                beta=np.ascontiguousarray(host_values["beta"][i]),
                standard_error=np.ascontiguousarray(host_values["standard_error"][i]),
                chi_squared=np.ascontiguousarray(host_values["chi_squared"][i]),
                log10_p_value=np.ascontiguousarray(host_values["log10_p_value"][i]),
                is_valid=np.ascontiguousarray(host_values["valid_mask"][i]),
            )
        )
    return payloads
...
def concatenate_linear_results(
    ...
    # Performance optimization: Passing a dict of lists directly to jax.device_get
    # avoids the significant tracing and JIT overhead of jnp.concatenate for variable
    # sized array lists, and reduces peak device memory usage. Concatenating on the
    # CPU with np.concatenate after transferring is much faster.
    host_values = jax.device_get(
        {
...
def concatenate_logistic_results(
    ...
    method_code_concat = np.concatenate(host_values["method_code"])
    error_code_concat = np.concatenate(host_values["error_code"])
    ...
    return pl.DataFrame(
        {
            ...
            "p_value": np.concatenate(host_values["p_value"]),
            "firth_flag": format_logistic_method_codes(method_code_concat),
            "error_code": format_logistic_error_codes(error_code_concat),
