...
def build_regenie2_linear_chunk_payload_batch(
    ...
    raw_host_values = jax.device_get(
        {
    ...
    host_values = {key: np.concatenate(value) for key, value in raw_host_values.items()}
...
def concatenate_linear_results(
    ...
    # Do ONE device_get on lists of arrays, then concatenate on host
    raw_host_values = jax.device_get(
        {
