...
impl NativeCliRunResult {
    fn new(exit_code: i32, stdout_chunks: Vec<String>, stderr_chunks: Vec<String>) -> Self {
        Self { exit_code, stdout_chunks, stderr_chunks }
    }
}
...
pub(crate) fn run_cli_with_python_backend(
    ...
    let Some(config) = config else {
        return Ok(NativeCliRunResult::new(exit_code, text_chunks(stdout), text_chunks(stderr)));
    };
    ...
    let context = match NativeCliRunContext::new(py, &config) {
        ...
        Err(error) => {
            ...
            let exit_code = append_terminal_result(&mut stdout_chunks, &mut stderr_chunks, terminal_result);
            return Ok(NativeCliRunResult::new(exit_code, stdout_chunks, stderr_chunks));
        }
    };
    ...
    let exit_code = append_terminal_result(&mut stdout_chunks, &mut stderr_chunks, close_result);
    Ok(NativeCliRunResult::new(exit_code, stdout_chunks, stderr_chunks))
}
...
fn run_backend_lifecycle(
    ...
    stdout_chunks: &mut Vec<String>,
    stderr_chunks: &mut Vec<String>,
) -> PyResult<Py<PyAny>> {
    {
        let context_reference = context.bind(py).borrow();
        initialize_cli_logging(py, config, context_reference.paths())?;
    }
    append_text_chunk(stdout_chunks, stdout);
    append_text_chunk(stderr_chunks, stderr);
...
fn completed_terminal_result(artifacts: &Bound<'_, PyAny>) -> PyResult<native_runtime::CliTerminalResult> {
    ...
    runtime::record_completed_terminal_lines(&stdout_lines)?;
    Ok(native_runtime::CliTerminalResult::success(stdout_lines))
}
...
fn interrupted_terminal_result(
    ...
    runtime::record_interrupted_terminal_lines(&stderr_lines)?;
    Ok(native_runtime::CliTerminalResult::interrupted(
        i32::try_from(interrupted_event.exit_code)
...
fn failed_terminal_result(
    ...
    Ok(native_runtime::CliTerminalResult::failed(runtime::render_and_record_failed_terminal_lines(&failed_event)))
}
...
fn telemetry_close_failure_result(
    ...
    let close_failure_plan = native_runtime::plan_cli_telemetry_close_failure(
        current_exit_code,
        native_runtime::CLI_RUNTIME_FAILURE_EXIT_CODE,
    );
    if !close_failure_plan.should_report_failure {
        return Ok(native_runtime::CliTerminalResult::empty(close_failure_plan.exit_code));
    }
    let failed_event = run_events::run_failed_event_payload_from_error(error.value(py))?;
    Ok(native_runtime::CliTerminalResult::new(
        close_failure_plan.exit_code,
        Vec::new(),
        runtime::render_and_record_failed_terminal_lines(&failed_event),
    ))
}
...
fn append_terminal_result(
    stdout_chunks: &mut Vec<String>,
    stderr_chunks: &mut Vec<String>,
    terminal_result: native_runtime::CliTerminalResult,
) -> i32 {
    stdout_chunks.extend(lines_to_chunks(terminal_result.stdout_lines));
    stderr_chunks.extend(lines_to_chunks(terminal_result.stderr_lines));
    terminal_result.exit_code
}

fn text_chunks(text: String) -> Vec<String> {
    if text.is_empty() { Vec::new() } else { vec![text] }
}

fn append_text_chunk(chunks: &mut Vec<String>, text: &str) {
    if !text.is_empty() {
