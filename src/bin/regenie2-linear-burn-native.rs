...
use _core::native::cubecl_linear;
...
struct Cli {
    ...
    cuda_block_size: u32,
    #[arg(long = "cubecl-block-size", default_value_t = 256)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.backend {
        ...
        NativeBackend::CubeclKernelWgpu => run_cubecl_kernel_wgpu(cli)?,
...
fn run_cubecl_kernel_wgpu(cli: Cli) -> Result<NativeRunReport, Box<dyn std::error::Error>> {
    #[cfg(feature = "burn-cubecl-kernel")]
    {
        type Backend = burn_cubecl::CubeBackend<burn::backend::wgpu::WgpuRuntime, f32, i32, u32>;
        let device = Default::default();
        return run_with_cubecl_kernel_backend::<Backend>(cli, &device, "cubecl-kernel-wgpu");
    }
    #[cfg(not(feature = "burn-cubecl-kernel"))]
    {
        let _ = cli;
        Err("The native binary was not built with the burn-cubecl-kernel feature.".into())
    }
}
...
#[cfg(feature = "burn-cubecl-kernel")]
fn run_with_cubecl_kernel_backend<Backend: cubecl_linear::LinearCubeclBackend>(
    cli: Cli,
    device: &Backend::Device,
    backend_name: &str,
) -> Result<NativeRunReport, Box<dyn std::error::Error>> {
    ...
    let mut current_chromosome = String::new();
    let mut current_chromosome_state: Option<cubecl_linear::LinearCubeclDeviceChromosomeState<Backend>> = None;
    let mut processed_variant_count = 0usize;
    let mut checksum = 0.0_f64;
    for chunk_spec in &chunk_specs {
        let variant_count = chunk_spec.variant_stop_index - chunk_spec.variant_start_index;
        ...
        if current_chromosome_state.is_none() || chromosome != current_chromosome {
            ...
            current_chromosome_state = Some(timer.measure("chromosome_state_prepare", || {
                let chromosome_state =
                    linear::prepare_linear_chromosome_state(prepared_linear_state, prediction_values)?;
                Ok::<_, linear::LinearError>(cubecl_linear::prepare_linear_cubecl_device_chromosome_state::<Backend>(
                    &chromosome_state,
                    device,
                ))
            })?);
        ...
        let compute_start_time = Instant::now();
        let result = cubecl_linear::compute_linear_chunk_cubecl::<Backend>(
            current_chromosome_state.as_ref().ok_or("Chromosome state was not prepared.")?,
            genotype_values,
            variant_count,
            device,
            cli.cubecl_block_size,
        )?;
        timer.add_seconds("cubecl_upload_compute_materialize_cpu_pvalue", compute_start_time.elapsed().as_secs_f64());
        for (stage_name, seconds) in &result.timing_seconds {
            timer.add_seconds(stage_name, *seconds);
        }
        checksum += result.checksum;
    ...
    let report = NativeRunReport {
        ...
        final_parquet,
        cuda_block_size: None,
        cubecl_block_size: Some(cli.cubecl_block_size),
        stage_seconds: timer.stage_seconds,
        total_wall_seconds,
    };
    if let Some(report_path) = report_json_path {
