...
use _core::output::OutputWriterSession;
use _core::regenie;
use _core::sample::SampleKeyMode;
use clap::{Parser, ValueEnum};
use serde::Serialize;

const DEFAULT_CHUNK_SIZE: usize = 65_536;
const DEFAULT_CHUNKS_PER_ARROW_FILE: usize = 1;
const DEFAULT_ARROW_COMPRESSION: &str = "zstd";
...
#[derive(Debug, Parser)]
#[command(name = "regenie2-linear-native")]
struct Cli {
    #[arg(long)]
    bgen: PathBuf,
    #[arg(long)]
...
fn run_with_cuda_kernel(cli: &Cli) -> Result<NativeRunReport, Box<dyn std::error::Error>> {
    ...
    let writer_session = match cli.output_mode {
        OutputMode::FullParquet | OutputMode::ChunksOnly => Some(timer.measure("output_writer_prepare", || {
            OutputWriterSession::new(
                ...
                DEFAULT_CHUNKS_PER_ARROW_FILE,
                DEFAULT_ARROW_COMPRESSION.to_string(),
                true,
            )
        })?),
    ...
    for chunk_spec in &chunk_specs {
        ...
        if current_chromosome_state.is_none() || chromosome != current_chromosome {
            current_chromosome.clone_from(&chromosome);
        ...
        if let Some(writer) = writer_session.as_ref() {
            ...
            writer.write_regenie2_native_chunk(
                i64::try_from(chunk_spec.variant_start_index).unwrap_or(i64::MAX),
                i64::try_from(chunk_spec.variant_stop_index).unwrap_or(i64::MAX),
                &metadata,
                &chunk_stats,
                &result.beta,
...
fn build_run_directory(output_prefix: &Path) -> PathBuf {
    PathBuf::from(format!("{}.regenie2_linear_native.run", output_prefix.display()))
}
