...
# What is especially good in this rewrite

These are decisions I would keep:

## Rust-native sample alignment

Moving alignment into Rust is the right performance direction. The remaining work is to make the parser streaming/selected-column, not to go back to DataFrames.

## Trusted variant-major BGEN path

The trusted path now looks much closer to the right shape:

```text
decode BGEN
write variant-major dosage
accumulate sums/squares/counts
send stats to JAX
```

This is a meaningful performance improvement over decode-then-scan.

## Score-only binary default

This fixes a major benchmarking trap. Binary default should be score-only, with approximate Firth only when requested.

## Config/TOML support

The TOML round-trip shape is good. I tested a minimal config with `[g.diagnostics]`, and it round-tripped correctly when using the `g-stage-timings-json` option name.

## Preflight validation

`src/g/engine/preflight.py` is a good addition. It validates finite arrays, covariate rank, binary case/control counts, and prediction shape. I would extend it, not remove it.

---

# Recommended rewrite plan

## Sprint 1: correctness hardening

1. Fix Firth convergence and add step-halving.
2. Surface null logistic non-convergence and fail by default.
3. Remove `allow_duplicate_iid_alignment`.
4. Replace mutable global JAX kernel constants with explicit static kernel config.
5. Harden resume manifest compatibility before accepting committed chunks.
6. Add tests for resume incompatibility, duplicate IID rejection, Firth convergence, and repeated Python API runs with different kernel settings.

## Sprint 2: architecture cleanup

1. Introduce `ExecutionPlan`.
2. Move orchestration out of `api.py` into `runner.py`.
3. Generate Click options from `OptionSpec`.
4. Write effective config before run start.
5. Add structured JSON logging/tracing instead of only `stage_timings_json`.
6. Fix callback shutdown behavior.

## Sprint 3: performance-critical data path

1. Replace `read_to_string + Vec<Vec<String>>` table parsing with streaming selected-column TSV parsing.
2. Build multi-phenotype alignment and prediction loading.
3. Rewrite the engine around multi-phenotype chunk processing.
4. Make writer support phenotype dimension or efficient split outputs.
5. Keep text output out of the benchmark path unless optimized.

## Sprint 4: GPU pipeline overlap

1. Add production async pipeline mode.
2. Avoid unconditional H2D and compute synchronization per chunk.
3. Use an in-flight result limit.
4. Keep blocking timings only for explicit profiling mode.
5. Benchmark staging depth, chunk size, writer depth, and output format together.

## Sprint 5: Firth parity and optimization

1. Build a Firth parity harness.
2. Compare batch sizes, block math, variant-major direct path, and sample-major path.
3. Only promote faster Firth variants after exact parity gates pass.
4. Consider Pallas/Triton only after this harness identifies a single kernel as the true bottleneck.

---

# Bottom line

The rewrite is moving in the right direction. The app now looks like a real pre-release engine rather than a prototype.

The biggest remaining issue is not “we need Triton” or “we need more micro-optimizations.” It is this:

```text
The architecture still processes one phenotype at a time,
while the GPU wants many phenotypes per decoded genotype chunk.
```

And before performance work, I would fix these correctness blockers:

```text
Firth convergence
stale JAX kernel config
resume manifest compatibility
duplicate IID alignment
null logistic non-convergence
```

Once those are fixed, the next major performance rewrite should be multi-phenotype execution. That is the path most likely to make `g` convincingly faster than REGENIE rather than just marginally faster on selected single-trait cases.
